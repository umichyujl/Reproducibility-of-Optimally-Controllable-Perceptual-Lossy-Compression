...
from default_config_A import hific_args, mse_lpips_args, directories, ModelModes, ModelTypes
...
def save_checkpoint(model, discr, epoch, gener, D_optimizer, G_optimizer):
    # model_out_path = "checkpoint_A/" + "model_epoch_{}.pth".format(epoch)
    model_out_path = "checkpoint_A_high_0.005/" + "model_epoch_{}.pth".format(epoch)
    state = {"epoch": epoch ,
             "model": model, 
             "discr": discr,
    ...
    if not os.path.exists("checkpoint_A_high_0.005/"):
        os.makedirs("checkpoint_A_high_0.005/")
...
def train(args, model, train_loader, test_loader, device, logger, G_optimizer=None, D_optimizer=None, gener=None, discr=None, optimizers=None):
    ...
    for epoch in trange(args.n_epochs, desc='Epoch'):
        ...
        for idx, (data, bpp) in enumerate(tqdm(train_loader, desc='Train'), 0):
            ...
            #第一个阶段 Rate, distortion, perceptual only
            ...
            # D_result = discr(target, latent).squeeze()
            ...
            # D_result = discr(G_result.data, latent).squeeze()
            ...
            # interpolated2 = Variable(latent, requires_grad=True)
            ...
            # D_result = discr(G_result, latent).squeeze()
            ...
            if iteration % args.log_interval == 0:
                ### use tensorboard ###
                train_writer.add_scalar('compression_loss', compression_loss.data, iteration)
                train_writer.add_scalar('loss_img', mse_loss.data, iteration)
...
if __name__ == '__main__':
    ...
    warmstart_args.add_argument("-ckpt", "--warmstart_ckpt", \
      default="/content/gdrive/MyDrive/RGBcode/experiments/FA_high/checkpoints/FA_high_epoch230_idx24380.pt", help="Path to autoencoder + hyperprior ckpt.")
    ...
    if args.use_exist_weight: 
        _, gener, discr, D_optimizer, G_optimizer = load_checkpoint(args.pr_path)
