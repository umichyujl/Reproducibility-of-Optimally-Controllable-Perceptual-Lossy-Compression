...
from srresnet_unet_CAB import _NetG,_NetD #, _NetG_bn
...
def train(args, model, gener, discr, train_loader, test_loader, device, logger, G_optimizer, D_optimizer):
    ...
    for epoch in trange(args.n_epochs, desc='Epoch'):
        ...
        # if epoch > 0:
        if (epoch+1) % args.save_interval == 1:
        ...
        for idx, (data, bpp) in enumerate(tqdm(train_loader, desc='Train'), 0):
            ...
            ## 加
            if model.use_discriminator == False:
             print("===> Epoch[{}]({}): Loss_img: {:.5}".format(epoch, iteration, compression_loss.item()))
...
def save_checkpoint(model, discr, epoch):
    model_out_path = "/content/gdrive/MyDrive/RGBcode/checkpoint/" + "model_epoch_{}.pth".format(epoch)
...
if __name__ == '__main__':
    ...
    general.add_argument("-save_intv", "--save_interval", type=int, default=200, help="Number of steps between checkpoints.")
    ...
    general.add_argument('--save', type=str, default='/content/gdrive/MyDrive/RGBcode/experiments', help='Parent directory for stored information (checkpoints, logs, etc.)')
...
###### check warmstart #########
    print("warmstart is: ", args.warmstart)
################################
...
###########  need to create train/test/val folders for loading datasets  
    #print(os.path.join(args.dataset_path, "train"))
############ 
    #print(datasets.BaseDataset.__len__)
...
############# succeed!
    ...
    Train
    """
    ############## need some parameters (previously missing)
