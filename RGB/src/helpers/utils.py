...
def setup_generic_signature(args, special_info):
    ...
    # args.name = "weights_gan"
    print(args.name)
    args.snapshot = os.path.join('/content/gdrive/MyDrive/RGBcode/experiments', args.name)
...
def save_model(model, optimizers, mean_epoch_loss, epoch, device, args, logger, multigpu=False):
    ...
    metadata_path = os.path.join(directory, 'metadata/model_{}.json'.format(model_name))
...
def load_model(save_path, logger, device, model_type=None, model_mode=None, current_args_d=None, prediction=True, 
    ...
    if prediction is True:
        ...
        # breakpoint()
        # optimizers['amort'].param_groups
        # checkpoint['compression_optimizer_state_dict']['param_groups']
