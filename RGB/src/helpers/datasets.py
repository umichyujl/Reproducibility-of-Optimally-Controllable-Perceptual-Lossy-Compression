...
class BaseDataset(Dataset, abc.ABC):
    ...
    def __init__(self, root, transforms_list=[], mode='train', logger=logging.getLogger(__name__),
        ...
        self.logger = logger
        ########## add for testing the paths #########
        #print(os.path.join(root, self.files["train"]))
        #print(os.path.join(root, self.files["test"]))
        #print(os.path.join(root, self.files["val"]))
        ##########
        if not os.path.isdir(root):
            raise ValueError('Files not found in specified directory: {}'.format(root))

    def __len__(self):
        print("length of images: ", len(self.imgs))
...
class Evaluation(BaseDataset):
    ...
    def __init__(self, root=os.path.join(DIR, 'data'), normalize=False, **kwargs):
        ...
        self.imgs += glob.glob(os.path.join(root, '*.bmp'))
