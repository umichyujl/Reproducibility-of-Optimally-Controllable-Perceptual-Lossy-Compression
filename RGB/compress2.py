...
def prepare_dataloader(args, input_dir, output_dir, batch_size=1):
    ...
    input_images += glob.glob(os.path.join(input_dir, '*.bmp'))
...
def compress_and_decompress(args, a, b):
    ...
    gener = torch.load(args.decoder)["gener"]
    gener.eval()
    ...
    with torch.no_grad():
        ...
        for idx, (data, bpp, filenames) in enumerate(tqdm(eval_loader), 0):
            ...
            if args.reconstruct is True:
                ...
                for i_h in range(num_h):
                    ...
                    for i_w in range(num_w):
                        ...
                        # a, _ = model.compression_forward(data)
                        # r = a.reconstruction
                        # l = a.latents_quantized
                        # r_d = gener(l)
                        # rr = r*0.25 + r_d*0.75
                        ...
                        #####
                        # mse,_ = model.compression_forward(patch_data)
                        #####

                        final_out[:,:,start_h:end_h,start_w:end_w]=a*reconstruction + b*reconstruction_mse
                ...
                # r_d = r_d[:,:,:rec_dim[2], :rec_dim[3]]
                # final_out = 0.8*r_d + 0.2*r
            ...
            if args.metrics is True:
                ...
                ms_ssim = MS_SSIM_func(reconstruction.cpu() * max_value, data.cpu() * max_value)
...
def main(**kwargs):
    ...
    parser.add_argument("-i", "--image_dir", type=str, default='/content/gdrive/MyDrive/RGBcode/data/coco2014/test',
        help="Path to directory containing images to compress")
    # parser.add_argument("-i", "--image_dir", type=str, default='/content/gdrive/MyDrive/RGBcode/data/G10',
    # parser.add_argument("-i", "--image_dir", type=str, default='/content/gdrive/MyDrive/RGBcode/data/',
        # help="Path to directory containing images to compress")
    parser.add_argument("-o", "--output_dir", type=str, default='/content/gdrive/MyDrive/RGBcode/data/L0.005', #Kodak_compression
    ...
    # parser.add_argument("--decoder", default="/content/gdrive/MyDrive/RGBcode/checkpoint_high/model_epoch_270.pth", type=str, help="decoder path")
    parser.add_argument("--decoder", default="/content/gdrive/MyDrive/RGBcode/checkpoint_A_high_0.005/model_epoch_45.pth", type=str, help="decoder path")
    
    args = parser.parse_args()

    input_images = glob.glob(os.path.join(args.image_dir, '*.jpg'))
    input_images += glob.glob(os.path.join(args.image_dir, '*.png'))
    input_images += glob.glob(os.path.join(args.image_dir, '*.bmp'))
    ...
    compress_and_decompress(args, 0, 1)
