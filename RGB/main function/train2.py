        ...
        for idx, (data, bpp) in enumerate(tqdm(train_loader, desc='Train'), 0):
            ...
            try:
                if model.use_discriminator is True:
                    ...
                    with torch.no_grad():
                        ################### revise fixE to freezE ##################
                        reconstruction,_ = model.compression_forward(data, freezE=True)
            ...
            if iteration % args.log_interval == 1:
                ...
                for subidx in range(G_result.shape[0]):
                    fname = os.path.join('/content/gdrive/MyDrive/RGBcode/data/reconstruction', "{}.png".format(saveflag))
                    torchvision.utils.save_image(G_result[subidx], fname, normalize=False)
                    fname = os.path.join('/content/gdrive/MyDrive/RGBcode/data/reconstruction', "{}_inp.png".format(saveflag))
...
def save_checkpoint(gener, discr, epoch):
    model_out_path = "/content/gdrive/MyDrive/RGBcode/checkpoint/" + "model2_epoch_{}.pth".format(epoch)
...
if __name__ == '__main__':
    ...
    general.add_argument("--resume", default="/content/gdrive/MyDrive/RGBcode/experiments/checkpoints/model_epoch_39.pth", type=str, help="Path to checkpoint (default: none)")
    ...
    ########## check ################
    print("warmstart: ", args.warmstart)
    print(dictify(args))
    ###########################################
    ...
    """
    ############# comment ckpt_path ###############
    model = train(args, model, gener, discr, train_loader, test_loader, device, logger, G_optimizer, D_optimizer)
    #, ckpt_path
    """
    TODO
    Generate metrics
